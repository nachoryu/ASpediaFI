...
#'@param expr a SummarizedExperiment object or matrix containing gene expression
#'profiles (FPKM)
...
analyze <- function(query, psi, expr, pathways = NULL, ppi = NULL,
    ...
    edges$weight = edges$weight / typetable[as.character(edges$type),
                                            "weight"]
    vertices <- data.frame(
        name = c(gene.nodes, as.nodes,pathway.nodes),
