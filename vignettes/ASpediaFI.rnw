...
rankings of genes and features.
...
AS event annotations from a GRCh38 GTF file using the \texttt{annotateASevents}
method. Due to file size limitations, we extract AS event annotations from a
subset of GRCh38 GTF file provided in the \texttt{extdata} directory of the
package.
...
GSE114922.ASpediaFI$annotateASevents(gtf.file = gtf, num.cores = 3)
...
file and save in a \texttt{gtf} field as a \texttt{GRanges} object for the
...
the fraction of mRNAs containing the alternatively spliced exon [4]. It
requires the \texttt{samples} field to have three
...
values in the \texttt{psi} field as a \texttt{SummarizedExperiment} object with
sample information. Note that row names of PSI values are AS event IDs.
...
\texttt{samples} fields with PSI values and sample information stored in the
example dataset.
...
design <- cbind(WT = 1, MTvsWT = GSE114922.ASpediaFI$samples$condition == "MUT")
fit <- lmFit(log2(GSE114922.fpkm + 1), design = design)
fit <- eBayes(fit, trend = TRUE)
tt <- topTable(fit, number = Inf, coef = "MTvsWT")
...
heterogeneous network. Please see help(ASpediaFI) for details.
...
GSE114922.ASpediaFI$analyzeFI(query = query, expr = GSE114922.fpkm,
                              restart = 0.9, num.folds = 5, num.feats = 100,
...
AS event ID, gene symbol, AS event type, final ranking, stationary probability,
and PSI values of samples in each condition.
<<tables,message=FALSE>>=
#Table of AS nodes in the final subnetwork
GSE114922.ASpediaFI$as.table[1:10, 2:5]

#Table of GS nodes in the final subnetwork
GSE114922.ASpediaFI$pathway.table[1:5, 1:11]
@
...
\item \texttt{Probability}: stationary probability
...
library(igraph)
edges <- as_data_frame(GSE114922.ASpediaFI$network)
...
GSE114922.ASpediaFI$exportNetwork(node = "HALLMARK_HEME_METABOLISM",
...
#Visualize AS event
GSE114922.ASpediaFI$visualize(node = GSE114922.ASpediaFI$as.table$EventID[10],
                              zoom = FALSE)
@
...
GSE114922.ASpediaFI$visualize(node = "HALLMARK_HEME_METABOLISM", n = 10)
@
